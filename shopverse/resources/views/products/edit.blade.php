<x-layout title="ShopVerse - Edit Produk">
    <div class="py-12 bg-slate-950">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight">Edit Produk</h1>
                    <p class="text-slate-400 text-sm mt-1">Ubah rincian informasi produk #{{ $product->id }}.</p>
                </div>
                ...
                <form action="{{ route('products.update', $product->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" placeholder="Contoh: Wireless Noise-Canceling Headphones" required
                        ...
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" placeholder="1499000" min="0" step="1" required
                        ...
                        <div>
                            ...
                            <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" placeholder="10" min="0" step="1" required
                        ...
                        <div>
                            ...
                            <input type="url" name="image" id="image" value="{{ old('image', $product->image) }}" placeholder="https://images.unsplash.com/..."
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="4" placeholder="Tuliskan spesifikasi dan penjelasan lengkap mengenai produk..."
                                class="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm transition-all @error('description') border-rose-500 @enderror">{{ old('description', $product->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-xs text-rose-400">{{ $message }}</p>
                            @enderror
                        </div>
